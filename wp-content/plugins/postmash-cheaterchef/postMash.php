...
function postMash_getPages(){
	...
	//Removing post types from query
	...
	if (isset($_GET['cat']) && $_GET['cat'] != '0') {
		...
		if (isset($_GET['m']) && $_GET['m'] != '0' ) {
			$query_post .= " AND YEAR($wpdb->posts.post_date) = " . $yyear ." 
				AND MONTH($wpdb->posts.post_date) = " . $mmonth ;
		}		
...
/* BEGIN ID/LAB CUSTOM CODE */
function my_wp_dropdown_posttypes($args) {
	$defaults = array(
		'pt_args'		=> array (
				'public'   => true,
				'_builtin' => false
	...
	if ( get_bloginfo('url') == "http://idlab.com.au/" ) {
...
function postMash_main(){
	...
	<div id="postMash" class="wrap">
		...
		<p style="margin-top:4px;">
			...
			<?php
			/* BEGIN ID/LAB CUSTOM CODE */
			$post_type = $_GET['page_id'];
			$dropdown_options = array('show_option_all' => __('View all post types'), 'hide_empty' => 0, 'hierarchical' => 1, 'show_count' => 0, 'orderby' => 'name', 'selected' => $post_type);
			my_wp_dropdown_posttypes($dropdown_options);
...
function postMash_add_pages(){
	...
	if($wp_version >= 2.7){
		$page = add_submenu_page('edit.php', 'postMash Custom: Order Posts', 'Reorder content', $minlevel,  __FILE__, 'postMash_main'); 
	}else{
		$page = add_management_page('postMash Custom: Order Posts', 'Reorder content', $minlevel, __FILE__, 'postMash_main');
	}
