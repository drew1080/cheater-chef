...
if(!function_exists('templatic_twitter_messages')){
function templatic_twitter_messages($options) {
	
	// CHECK OPTIONS
	
	if ($options['username'] == '') {
		return __('Twitter username not configured','supreme');
	} 
	
	if (!is_numeric($options['num']) or $options['num']<=0) {
		return __('Number of tweets not valid','supreme');
	}
	...
	if (!$twitter_data) {
		...
		$json = wp_remote_get('http://api.twitter.com/1/statuses/user_timeline.json?screen_name='.$options['username'].'&count='.$max_items_to_retrieve);
 		if( is_wp_error( $json ) ) {
			return __('Error retrieving tweets','supreme');
		} else {
	...
	if (empty($twitter_data) and false === ($twitter_data = get_transient($transient_name."_valid"))) {
	    return __('No public tweets','supreme');
	}
	...
	if (isset($twitter_data['errors'])) {
	    ...
	    if(false === ($twitter_data = get_transient($transient_name."_valid"))) {
		    ...
		    return sprintf(__('Unable to get tweets %s','supreme'),$debug);
		}
	}
	...
	foreach($twitter_data as $message){
		...
		if (isset($options['twitter_users']) && $options['twitter_users'] != "")  { 
...
class templatic_twiter extends WP_Widget {
    ...
    function templatic_twiter() {
		$this->options = array(
			...
			array(
				'name'	=> 'num',
				'label'	=> __( 'Show # of Tweets', 'supreme' ),
				'type'	=> 'text'
			),
			array(
				'name'	=> 'linked',
				'label'	=> __( 'Show this linked text for each Tweet', 'supreme' ),
				'type'	=> 'text'
			),
			array(
				'name'	=> 'follow_text',
			...
			array(
				'name'	=> 'link_user',
				'label'	=> __( 'Link below tweets with Twitter user', 'supreme' ),
				'type'	=> 'checkbox'
			),			
			
			array(
				'name'	=> 'hyperlinks',
				'label'	=> __( 'Find and show hyperlinks', 'supreme' ),
				'type'	=> 'checkbox'
			),			
        ...
        $widget_ops = array('classname' => __('widget Templatic twitter','supreme'), 'description' => __('Show your latest tweets on your site. .','supreme') );
		$this->WP_Widget('templatic_twiter', __('T &rarr; Latest Twitter Feed','supreme'), $widget_ops);
    }
    ...
    function widget($args, $instance) {		
		...
		_e(templatic_twitter_messages($instance),'supreme');
		if(isset($instance['follow_text']) && $instance['follow_text'] !="" ){
            
			echo '<a href="http://twitter.com/' . $instance['username'] . '" class="twitter_title_link follow_us_twitter" '.$link_target.'>'.  sprintf(__('%s','supreme'),$follow_text) . '</a>';
    ...
    function form($instance) {
		if (empty($instance)) {
			$instance['title']			= __( 'Last Tweets', 'supreme' );			
			$instance['username']		= '';
			$instance['num']			= '5';			
			$instance['linked']			= '#';
...
// register ReallySimpleTwitterWidget 
...
class templatic_slider extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => __('widget Templatic Slider','supreme'), 'description' => __('Home page post slider with display selected post type or display custom images on home page slider','supreme') );
	...
	<p>
		<label for="<?php echo $this->get_field_id('slideshowSpeed'); ?>">
			...
			<input class="widefat" id="<?php echo $this->get_field_id('slideshowSpeed'); ?>" name="<?php echo $this->get_field_name('slideshowSpeed'); ?>" type="text" value="<?php echo esc_attr($slideshowSpeed); ?>" />
			  <small><?php _e('Default : 7000 - Set the speed of the slideshow cycling, in milliseconds.','supreme');?> </small>
		</label>
	</p>
	<p>
...
<div id="<?php echo $this->get_field_id('home_slide_carousel'); ?>" style="<?php if($is_Carousel =='1'){ ?>display:block;<?php }else{?>display:none;<?php }?>">
	...
	<p>
		<label for="<?php echo $this->get_field_id('item_move'); ?>">
			<?php _e('Items Move <br/><small>(Number of items that should move on animation. If 0, slider will move all visible items.)</small>','supreme'); ?>
...
class supreme_popular_post extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => __('widget Templatic Popular Posts Widget ','supreme'), 'description' => __('Widget list the popular posts as per total views , daily views or comments.( you can also select another post-type)','supreme') );
...
class templatic_recent_post extends WP_Widget {
	...
	//Constructor
		$widget_ops = array('classname' => __('widget Templatic Listing Post','supreme'), 'description' => __('Show Listing post with post thumbnail, post title, post content, post category, gravatar with order by option.','supreme') );
	...
	function widget($args, $instance) {
		...
		print_r($featured_posts->request);
